...
if(!isset($_SESSION['user_id'])){
    die(json_encode(['resultSuccess' => false, 'error' => 'User is not logged in']));
}
...
try{
    ...
    die(json_encode(['resultSuccess' => false, 'error' => 'Wrong date']));
}
...
if($ptAPI::checkIfUserIsPowerTrailOwner($_SESSION['user_id'], $powerTrailId) == 1) {
    ...
    die(json_encode(['resultSuccess' => true, 'error' => null]));
} else {
    die(json_encode(['resultSuccess' => false, 'error' => 'User is not powerTrail owner']));
}
